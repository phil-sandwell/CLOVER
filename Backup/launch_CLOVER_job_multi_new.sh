...
USERNAME=$USER
CLOVER_PATH="/home/${USERNAME}/CLOVER-hpc/"
...
These jobs must contain the a line 'location=LOCATION_NAME_HERE' in the first few lines so that the script knows where to get Location details from.cp -r ${CLOVER_PATH}/CLOVER-master//Locations/$LOCATION/ CLOVER/Locations/
...
mkdir CLOVER/
cp -r ${CLOVER_PATH}/CLOVER-master/Scripts CLOVER/
...
do
 ...
 if [ ! -d CLOVER/Locations/$LOCATION/ ]
  then
  cp -r ${CLOVER_PATH}/CLOVER-master/Locations/$LOCATION/ CLOVER/Locations/
 fi
