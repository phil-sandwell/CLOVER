...
#   Format and save output summary
    ...
    def optimisation_step(self,PV_sizes=[],storage_sizes=[],previous_systems = pd.DataFrame([]), start_year = 0):
        '''
        Function:
...
#   Increase  and iterate over PV size
            ...
            while iteration_PV_size >= PV_size_min:
                simulation = Energy_System(**self.kwargs).simulation(start_year = start_year, end_year = end_year, 
...
# =============================================================================
    def simulation_technical_appraisal(self,simulation,**kwargs):
        '''
        Function:
            Appraises the technical performance of a minigrid system
        Inputs:
            simulation          Outputs of Energy_System(**self.kwargs).simulation(...)
...
#   Calculate operating costs of the system during this simulation (discounted)
        OM_costs = Finance(**self.kwargs).get_total_OM(
...
#   Calculate running costs of the system (discounted)
        diesel_costs = Finance(**self.kwargs).get_diesel_fuel_expenditure(
                diesel_fuel_usage_hourly = simulation_results['Diesel fuel usage (l)'],
                start_year=start_year,end_year=end_year)
        grid_costs = Finance(**self.kwargs).get_grid_expenditure(
                grid_energy_hourly = simulation_results['Grid energy (kWh)'],
                start_year=start_year,end_year=end_year)
        kerosene_costs = Finance(**self.kwargs).get_kerosene_expenditure(
                kerosene_lamps_in_use_hourly = simulation_results['Kerosene lamps'],
                start_year=start_year,end_year=end_year)
        kerosene_costs_mitigated = Finance(**self.kwargs).get_kerosene_expenditure_mitigated(
...
#   Calculate new equipment GHGs
        equipment_GHGs = GHGs(**self.kwargs).get_total_equipment_GHGs(
                ...
                year=intallation_year) + GHGs(**self.kwargs).get_independent_GHGs(
                        start_year,end_year)
#   Calculate GHGs of connecting new households
        connections_GHGs = GHGs(**self.kwargs).get_connections_GHGs(
                households = simulation_results['Households'],
                year = intallation_year)
#   Calculate operating GHGs of the system during this simulation
        OM_GHGs = GHGs(**self.kwargs).get_total_OM(
                PV_array_size = simulation_details.loc['System details']['Initial PV size'],
                storage_size = simulation_details.loc['System details']['Initial storage size'],
                diesel_size = simulation_details.loc['System details']['Diesel capacity'],
                start_year = start_year,end_year = end_year)
#   Calculate running GHGs of the system
        diesel_GHGs = GHGs(**self.kwargs).get_diesel_fuel_GHGs(
                diesel_fuel_usage_hourly = simulation_results['Diesel fuel usage (l)'],
                start_year=start_year,end_year=end_year)
        grid_GHGs = GHGs(**self.kwargs).get_grid_GHGs(
                grid_energy_hourly = simulation_results['Grid energy (kWh)'],
                start_year=start_year,end_year=end_year)
        kerosene_GHGs = GHGs(**self.kwargs).get_kerosene_GHGs(
                kerosene_lamps_in_use_hourly = simulation_results['Kerosene lamps'],
                start_year=start_year,end_year=end_year)
        kerosene_GHGs_mitigated = GHGs(**self.kwargs).get_kerosene_GHGs_mitigated(
                kerosene_lamps_mitigated_hourly = simulation_results['Kerosene mitigation'],
                start_year=start_year,end_year=end_year)
